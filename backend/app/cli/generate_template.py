...
def create_introduction_sheet(ws):
    ...
    # Version info
    ws["A3"] = (
        f"Template Version: 1.0 | Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    )
    ...
    for line in instructions:
        ...
        if (
            ...
            or line.startswith("TIPS")
            or line.startswith("NEED")
        ):
...
def create_org_info_sheet(ws):
    ...
    for row_num, (field, value, notes) in enumerate(fields, 5):
        ...
        ws.cell(row=row_num, column=3, value=notes).font = Font(
    ...
    dv_industry = DataValidation(
        type="list", formula1='"' + ",".join(INDUSTRIES) + '"', allow_blank=True
    )
    ...
    dv_country = DataValidation(
        type="list", formula1='"' + ",".join(country_short) + '"', allow_blank=True
    )
    ...
    dv_emp = DataValidation(
        type="list", formula1='"' + ",".join(EMPLOYEE_RANGES) + '"', allow_blank=True
    )
...
def create_scope_1_1_sheet(ws):
    ...
    ws["A2"] = (
        "Fuel burned in boilers, furnaces, generators, heaters. Use Physical (liters, kWh) OR Spend (currency)."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ...
    examples = [
        [
            "Natural Gas",
            "Physical",
            "EXAMPLE: Office heating - Q1 2024",
            "15000",
            "kWh",
            "2024-01",
            "HQ Building",
    ...
    # Add dropdowns
    # Fuel type dropdown
    dv_fuel = DataValidation(
        type="list",
        formula1='"' + ",".join(FUEL_TYPES_STATIONARY) + '"',
        allow_blank=True,
    )
    ...
    dv_method = DataValidation(
        type="list",
        formula1='"' + ",".join(CALCULATION_METHODS) + '"',
        allow_blank=True,
    )
    ...
    all_units = (
        list(set(u for units in UNITS_BY_FUEL.values() for u in units)) + CURRENCIES
    )
    dv_unit = DataValidation(
        type="list", formula1='"' + ",".join(all_units) + '"', allow_blank=True
    )
...
def create_scope_1_2_sheet(ws):
    ...
    ws["A2"] = (
        "Company vehicles. Use Physical (km/liters) OR Spend (fuel cost). Select '(Fuel Only)' for fuel purchases without vehicle info."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:G2")
    ...
    examples = [
        [
            ...
            "Diesel",
            "Physical",
            "EXAMPLE: Sales team vehicle ABC-123",
            "25000",
            "km",
            "2024-01",
        ],
    ...
    dv_vehicle = DataValidation(
        type="list", formula1='"' + ",".join(VEHICLE_TYPES) + '"', allow_blank=True
    )
    ws.add_data_validation(dv_vehicle)
    dv_vehicle.add("A6:A109")

    dv_fuel = DataValidation(
        type="list", formula1='"' + ",".join(FUEL_TYPES_MOBILE) + '"', allow_blank=True
    )
    ws.add_data_validation(dv_fuel)
    dv_fuel.add("B6:B109")
    ...
    dv_unit = DataValidation(
        type="list",
        formula1='"' + ",".join(mobile_units_currency) + '"',
        allow_blank=True,
    )
    ws.add_data_validation(dv_unit)
    dv_unit.add("F6:F109")
...
def create_scope_1_3_sheet(ws):
    ...
    ws["A2"] = (
        "Refrigerant leaks, top-ups, equipment disposal. Use Physical (kg) OR Spend (purchase cost)."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ...
    examples = [
        [
            "R-410A",
            "Physical",
            "EXAMPLE: AC system refill - Building A",
            "3.5",
            "kg",
            "2024-03",
        ],
        [
            "R-134a",
            "Spend",
            "EXAMPLE: Refrigerant purchase invoice",
            "450",
            "USD",
            "2024-04",
        ],
    ...
    dv_ref = DataValidation(
        type="list", formula1='"' + ",".join(REFRIGERANT_TYPES) + '"', allow_blank=True
    )
    ...
    dv_unit = DataValidation(
        type="list", formula1='"kg,' + ",".join(CURRENCIES) + '"', allow_blank=True
    )
    ws.add_data_validation(dv_unit)
    dv_unit.add("E6:E57")
...
def create_scope_1_4_sheet(ws):
    ...
    ws["A2"] = (
        "Industrial process emissions. Use Physical (tonnes produced) OR Spend (material purchase cost)."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:F2")
    ...
    examples = [
        [
            "Cement",
            "Physical",
            "EXAMPLE: Portland cement production - Plant A",
            "25000",
            "tonnes",
            "2024-Q1",
        ],
        [
            "Iron & Steel (Integrated)",
            "Spend",
            "EXAMPLE: Steel raw material purchase",
            "1500000",
            "USD",
            "2024-Q1",
        ],
    ...
    dv_mat = DataValidation(
        type="list", formula1='"' + ",".join(PROCESS_MATERIALS) + '"', allow_blank=True
    )
    ...
    dv_unit = DataValidation(
        type="list", formula1='"tonnes,' + ",".join(CURRENCIES) + '"', allow_blank=True
    )
    ws.add_data_validation(dv_unit)
    dv_unit.add("E6:E37")
...
def create_scope_2_1_sheet(ws):
    ...
    ws["A2"] = (
        "Grid electricity. Use Physical (kWh) OR Spend (electricity bill amount). Select country for correct grid factor. For market-based, select a power producer or method."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ...
    dv_type = DataValidation(
        type="list", formula1='"' + ",".join(ELECTRICITY_TYPES) + '"', allow_blank=True
    )
    ...
    dv_country = DataValidation(
        type="list", formula1='"' + ",".join(country_codes) + '"', allow_blank=True
    )
    ws.add_data_validation(dv_country)
    ...
    dv_producer.error = (
        "Select a valid power producer or leave blank for location-based"
    )
    dv_producer.errorTitle = "Invalid Producer"
    ws.add_data_validation(dv_producer)
    dv_producer.add("D12:D111")

    dv_market_method = DataValidation(
        type="list", formula1='"' + ",".join(MARKET_METHODS) + '"', allow_blank=True
    )
    ...
    dv_unit = DataValidation(
        type="list", formula1='"kWh,' + ",".join(CURRENCIES) + '"', allow_blank=True
    )
    ws.add_data_validation(dv_unit)
    dv_unit.add("H6:H111")
...
def create_scope_2_2_sheet(ws):
    ...
    ws["A2"] = (
        "District heating and industrial steam. Use Physical (kWh) OR Spend (bill amount)."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:F2")
    ...
    examples = [
        [
            "District Heating",
            "Physical",
            "EXAMPLE: Office heating - Winter 2024",
            "35000",
            "kWh",
            "2024-Q1",
        ],
        [
            "Steam",
            "Physical",
            "EXAMPLE: Manufacturing process steam",
            "18000",
            "kWh",
            "2024-01",
        ],
    ...
    # Dropdowns
    dv_energy = DataValidation(
        type="list", formula1='"' + ",".join(HEAT_STEAM_TYPES) + '"', allow_blank=True
    )
...
def create_scope_2_3_sheet(ws):
    ...
    ws["A2"] = (
        "Chilled water and district cooling. Use Physical (kWh) OR Spend (bill amount)."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:F2")
    ...
    # Dropdowns
    dv_cooling = DataValidation(
        type="list", formula1='"' + ",".join(COOLING_TYPES) + '"', allow_blank=True
    )
...
def create_scope_3_4_sheet(ws):
    ...
    ws["A2"] = (
        "Transport of purchased goods. Enter origin/destination countries and distance will be auto-calculated if left blank."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:J2")

    ws["A3"] = (
        "If Distance (km) is blank, origin + destination countries are used to look up a default route distance."
    )
    ...
    examples = [
        [
            "Physical",
            "Sea - Container",
            "EXAMPLE: Raw materials from China",
            "120",
            "CN",
            "IL",
            "",
        ...
        [
            "Physical",
            "Road - HGV",
            "EXAMPLE: Components from Germany",
            "25",
            "DE",
            "IL",
            "4200",
    ...
    # Dropdowns
    dv_method = DataValidation(
        type="list", formula1='"Physical,Spend"', allow_blank=True
    )
    ws.add_data_validation(dv_method)
    dv_method.add("A7:A110")

    dv_mode = DataValidation(
        type="list", formula1='"' + ",".join(TRANSPORT_MODES) + '"', allow_blank=True
    )
    ...
    dv_currency = DataValidation(
        type="list", formula1='"' + ",".join(CURRENCIES) + '"', allow_blank=True
    )
...
def create_scope_3_7_sheet(ws):
    ...
    ws["A2"] = (
        "Employee travel between home and work. Enter per group/mode or per site."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ...
    dv_mode = DataValidation(
        type="list", formula1='"' + ",".join(COMMUTING_MODES) + '"', allow_blank=True
    )
    ws.add_data_validation(dv_mode)
    dv_mode.add("B6:B60")
...
def create_scope_3_8_sheet(ws):
    ...
    ws["A2"] = (
        "Emissions from leased buildings/assets not in Scopes 1 & 2. Use asset-specific data or spend-based method."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:L2")

    ws["A3"] = (
        "If tenant-specific Scope 1 & 2 data is available, enter it in the dedicated columns for more accurate calculations."
    )
    ws["A3"].font = Font(italic=True, size=10, color="336699")
    ...
    headers = [
        ...
        "Gas (kWh)",
        "Country",
        "Tenant Scope 1 CO2e (kg)",
        "Tenant Scope 2 CO2e (kg)",
        "Spend Amount",
        "Currency",
        "Year",
    ]
    ...
    examples = [
        ...
        [
            "Physical",
            "Data Center",
            "EXAMPLE: Cloud hosting facility",
            "800",
            "450000",
            "",
            "US",
        ...
        [
            "Spend",
            "Office",
            "EXAMPLE: Small satellite office",
            "",
            "",
            "",
            "UK",
    ...
    dv_building = DataValidation(
        type="list",
        formula1='"' + ",".join(LEASED_BUILDING_TYPES) + '"',
        allow_blank=True,
    )
...
def create_scope_3_1_sheet(ws):
    ...
    ws.merge_cells("A1:J1")
    ws["A2"] = (
        "Raw materials, components, office supplies, and services purchased. Use Physical, Spend, or Supplier-Specific."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:J2")
    ...
    examples = [
        [
            "Physical",
            "Steel",
            "EXAMPLE: Raw steel plates for production",
            "15000",
            "kg",
            "",
            "",
        ...
        [
            "Spend",
            "Office Supplies",
            "EXAMPLE: Annual office supplies",
            "",
            "",
            "25000",
            "USD",
        ...
        [
            "Supplier-Specific",
            "Plastic-PET",
            "EXAMPLE: PET bottles from supplier",
            "8000",
            "kg",
            "",
            "",
    ...
    dv_method = DataValidation(
        type="list", formula1='"' + ",".join(SCOPE3_METHODS) + '"', allow_blank=True
    )
    ws.add_data_validation(dv_method)
    ...
    dv_unit = DataValidation(
        type="list",
        formula1='"kg,tonnes,liters,m3,kWh,units,' + ",".join(CURRENCIES) + '"',
        allow_blank=True,
    )
    ws.add_data_validation(dv_unit)
    dv_unit.add("E6:E108")
...
def create_scope_3_2_sheet(ws):
    ...
    ws.merge_cells("A1:J1")
    ws["A2"] = (
        "Purchased capital equipment: vehicles, IT, buildings, machinery. Use Physical, Spend, or Supplier-Specific."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:J2")
    ...
    examples = [
        [
            ...
            "IT Equipment",
            "Laptop",
            "EXAMPLE: 50 laptops for employees",
            "50",
            "unit",
            "",
            "",
        ...
        [
            ...
            "Vehicles",
            "Medium Car",
            "EXAMPLE: Fleet vehicle purchase",
            "3",
            "unit",
            "",
            "",
    ...
    dv_cat = DataValidation(
        type="list",
        formula1='"' + ",".join(CAPITAL_GOODS_CATEGORIES) + '"',
        allow_blank=True,
    )
    ...
    dv_type = DataValidation(
        type="list",
        formula1='"' + ",".join(CAPITAL_GOODS_TYPES) + '"',
        allow_blank=True,
    )
    ws.add_data_validation(dv_type)
    dv_type.add("C6:C58")
    ...
    dv_unit = DataValidation(
        type="list",
        formula1='"unit,m2,kW,' + ",".join(CURRENCIES) + '"',
        allow_blank=True,
    )
    ws.add_data_validation(dv_unit)
    dv_unit.add("F6:F58")
...
def create_scope_3_5_sheet(ws):
    ...
    ws.merge_cells("A1:J1")
    ws["A2"] = (
        "Waste disposal and treatment. Enter waste type, treatment method, and weight."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:J2")
    ...
    examples = [
        ...
        [
            ...
            "Mixed/General",
            "Landfill",
            "EXAMPLE: Waste disposal contract",
            "",
            "",
            "12000",
            "USD",
    ...
    dv_waste = DataValidation(
        type="list", formula1='"' + ",".join(WASTE_TYPES) + '"', allow_blank=True
    )
    ws.add_data_validation(dv_waste)
    dv_waste.add("B6:B58")

    dv_treatment = DataValidation(
        type="list", formula1='"' + ",".join(TREATMENT_METHODS) + '"', allow_blank=True
    )
    ...
    dv_unit = DataValidation(
        type="list",
        formula1='"kg,tonnes,m3,' + ",".join(CURRENCIES) + '"',
        allow_blank=True,
    )
    ws.add_data_validation(dv_unit)
    dv_unit.add("F6:F58")
...
def create_scope_3_6_flights_sheet(ws):
    ...
    ws.merge_cells("A1:K1")
    ws["A2"] = (
        "Business air travel. Enter IATA airport codes (e.g., TLV, LHR, JFK) - distance is auto-calculated."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:K2")
    ws["A3"] = (
        "If the same airport appears as origin and destination (e.g., TLV→TLV), it will be treated as 0 emissions."
    )
    ws["A3"].font = Font(italic=True, size=10, color="336699")
    ...
    dv_cabin = DataValidation(
        type="list", formula1='"' + ",".join(CABIN_CLASSES) + '"', allow_blank=True
    )
    ws.add_data_validation(dv_cabin)
    dv_cabin.add("D7:D109")

    dv_trip = DataValidation(
        type="list", formula1='"' + ",".join(TRIP_TYPES) + '"', allow_blank=True
    )
...
def create_scope_3_6_hotels_sheet(ws):
    ...
    ws.merge_cells("A1:I1")
    ws["A2"] = (
        "Hotel stays during business travel. Enter number of nights, rooms, and country."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:I2")
...
def create_scope_3_6_other_travel_sheet(ws):
    ...
    examples = [
        [
            ...
            "Physical",
            "450",
            "EXAMPLE: London to Manchester round trip",
            "",
            "",
            "2024-03",
        ],
    ...
    dv_travel = DataValidation(
        type="list", formula1='"' + ",".join(TRAVEL_TYPES) + '"', allow_blank=True
    )
...
def create_scope_3_9_sheet(ws):
    ...
    ws.merge_cells("A1:K1")
    ws["A2"] = (
        "Transport of sold products to customers. Same structure as 3.4 but for downstream."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:K2")
...
def create_scope_3_10_sheet(ws):
    ...
    examples = [
        [
            ...
            "",
            "Melting/Smelting",
            "EXAMPLE: Steel plates for auto parts",
            "50000",
            "kg",
            "",
            "",
        ...
        [
            ...
            "",
            "",
            "EXAMPLE: Revenue from processed goods",
            "",
            "",
            "",
            "",
    ...
    dv_method = DataValidation(
        type="list", formula1='"Physical,Spend,Site-Specific"', allow_blank=True
    )
    ws.add_data_validation(dv_method)
    dv_method.add("A7:A58")

    dv_process = DataValidation(
        type="list", formula1='"' + ",".join(PROCESSING_TYPES) + '"', allow_blank=True
    )
    ws.add_data_validation(dv_process)
    dv_process.add("D7:D58")

    dv_unit = DataValidation(
        type="list", formula1='"kg,tonnes,liters,units"', allow_blank=True
    )
    ws.add_data_validation(dv_unit)
    dv_unit.add("G7:G58")
...
def create_scope_3_11_sheet(ws):
    ...
    ws.merge_cells("A1:L1")
    ws["A2"] = (
        "Emissions from the use of products sold by your company during their expected lifetime."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:L2")
    ...
    headers = [
        ...
        "Description",
        "Units Sold",
        "Lifetime Energy (kWh/unit)",
        "Lifetime (years)",
        "Lifetime Fuel (liters/unit)",
        "Fuel Type",
        "Energy Source",
        "Revenue",
        "Date",
    ...
    dv_method = DataValidation(
        type="list", formula1='"Physical,Fuel,Spend"', allow_blank=True
    )
    ws.add_data_validation(dv_method)
    dv_method.add("A7:A58")
    ...
    dv_type = DataValidation(
        type="list",
        formula1='"' + ",".join(PRODUCT_CATEGORIES_USE) + '"',
        allow_blank=True,
    )
    ws.add_data_validation(dv_type)
    dv_type.add("B7:B58")
    ...
    dv_fuel = DataValidation(
        type="list",
        formula1='"Petrol,Diesel,Natural Gas,LPG,Electricity"',
        allow_blank=True,
    )
    ws.add_data_validation(dv_fuel)
    dv_fuel.add("I7:I58")
...
def create_scope_3_12_sheet(ws):
    ...
    ws.merge_cells("A1:K1")
    ws["A2"] = (
        "Emissions from disposal and treatment of products sold by your company at end of life."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:K2")
    ...
    examples = [
        [
            ...
            "Recycling",
            "PET packaging",
            "EXAMPLE: Product packaging waste",
            "25000",
            "",
            "",
            "",
        ...
        [
            ...
            "Recycling",
            "Circuit boards",
            "EXAMPLE: E-waste from sold devices",
            "5000",
            "10000",
            "0.5",
            "",
    ...
    dv_disposal = DataValidation(
        type="list", formula1='"' + ",".join(DISPOSAL_METHODS) + '"', allow_blank=True
    )
...
def create_scope_3_13_sheet(ws):
    ...
    ws["A2"] = (
        "Assets owned by your company and leased to others. Enter energy data or use area-based estimates."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ...
    examples = [
        [
            ...
            "Office",
            "Office",
            "EXAMPLE: Leased office space - Floor 3",
            "85000",
            "kWh",
            "1200",
            "",
    ...
    dv_method = DataValidation(
        type="list", formula1='"Physical,Spend,Asset-Specific"', allow_blank=True
    )
    ws.add_data_validation(dv_method)
    dv_method.add("A7:A58")
    ...
    dv_asset = DataValidation(
        type="list",
        formula1='"' + ",".join(DOWNSTREAM_ASSET_TYPES) + '"',
        allow_blank=True,
    )
    ...
    dv_energy_unit = DataValidation(
        type="list", formula1='"kWh,MWh,m3,liters"', allow_blank=True
    )
...
def create_scope_3_14_sheet(ws):
    ...
    ws.merge_cells("A1:M1")
    ws["A2"] = (
        "Emissions from franchise operations not in your Scope 1 & 2 (reported by franchisor)."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
    ws.merge_cells("A2:M2")
    ...
    dv_method = DataValidation(
        type="list", formula1='"Physical,Spend,Franchise-Specific"', allow_blank=True
    )
    ws.add_data_validation(dv_method)
    dv_method.add("A7:A38")

    dv_type = DataValidation(
        type="list", formula1='"' + ",".join(FRANCHISE_TYPES) + '"', allow_blank=True
    )
    ws.add_data_validation(dv_type)
    dv_type.add("B7:B38")
...
def create_reference_sheet(ws):
    ...
    ws["A2"] = (
        "This sheet shows all valid values accepted by the system. Use these exact values in your data."
    )
    ws["A2"].font = Font(italic=True, size=10, color="666666")
...
def generate(
    output: str = typer.Option(
        "climatrix_import_template.xlsx", "--output", "-o", help="Output file path"
    )
):
