...
def _generate_monthly_activities(site: dict, year: int = 2025) -> list[dict]:
        ...
        activities.append(
            {
                ...
                "description": "HVAC refrigerant R-410A annual top-up",
                "quantity": round(r410a_kg, 2),
                "unit": "kg",
            ...
            activities.append(
                {
                    ...
                    "quantity": round(
                        quarterly_food_spend * random.uniform(0.8, 1.1), 0
                    ),
                    "unit": "USD",
                    "activity_date": date(year, month, 28),
                }
    ...
    monthly_short_haul_pkm = (
        employees * flights_per_employee / 12 * 800
    )  # avg 800 km short haul
    monthly_long_haul_pkm = (
        employees * flights_per_employee / 12 * 4000
    )  # avg 4000 km long haul
    ...
    monthly_hotel_nights = (
        employees * flights_per_employee / 12 * 2.5
    )  # avg 2.5 nights per trip
    for month in range(1, 13):
        travel_factor = 0.6 if month in (7, 8, 12) else 1.15
            ...
            activities.append(
                {
                    ...
                    "quantity": round(
                        monthly_hotel_nights * travel_factor * random.uniform(0.8, 1.2),
                        0,
                    ),
                    "unit": "nights",
                    "activity_date": date(year, month, 20),
    ...
    for month in range(1, 13):
        ...
        # Car commuters
        car_km = (
            employees * car_pct * avg_commute_km * 2 * working_days * vacation_factor
        )
        if car_km > 0:
            activities.append(
                {
                    ...
                    "quantity": round(car_km * random.uniform(0.9, 1.1), 0),
                    "unit": "km",
                    "activity_date": date(year, month, 28),
                }
            )
        ...
        # Public transport
        rail_km = (
            employees * rail_pct * avg_commute_km * 2 * working_days * vacation_factor
        )
        if rail_km > 1000:
            activities.append(
                {
                    ...
                    "quantity": round(rail_km * random.uniform(0.9, 1.1), 0),
                    "unit": "km",
                    "activity_date": date(year, month, 28),
                }
            )
        ...
        bus_km = (
            employees * bus_pct * avg_commute_km * 2 * working_days * vacation_factor
        )
        if bus_km > 1000:
            activities.append(
                {
                    ...
                    "quantity": round(bus_km * random.uniform(0.9, 1.1), 0),
                    "unit": "km",
                    "activity_date": date(year, month, 28),
                }
            )
...
async def seed_nice_demo(session: AsyncSession, force: bool = False):
    ...
    if existing_org:
        ...
        if act_ids:
            await session.execute(
                delete(Emission).where(Emission.activity_id.in_(act_ids))
            )
            await session.execute(
                delete(Activity).where(Activity.organization_id == existing_org.id)
            )
        await session.execute(
            delete(ImportBatch).where(ImportBatch.organization_id == existing_org.id)
        )
        await session.execute(
            delete(Site).where(Site.organization_id == existing_org.id)
        )
        await session.execute(
            delete(ReportingPeriod).where(
                ReportingPeriod.organization_id == existing_org.id
            )
        )
        await session.execute(
            delete(User).where(User.organization_id == existing_org.id)
        )
    ...
    for site_data in NICE_SITES:
        ...
        typer.echo(
            f"  {site_data['name']}: generating {len(site_activities)} activities..."
        )
    ...
    typer.echo(f"\n{'='*60}")
    typer.echo("NICE Ltd Demo Seeding Complete")
    typer.echo(f"{'='*60}")
    typer.echo("Organization: NICE Ltd")
    typer.echo("Login:        demo@nice.com / NiceDemo2025!")
    typer.echo(f"Sites:        {len(NICE_SITES)}")
    typer.echo("Period:       FY 2025")
