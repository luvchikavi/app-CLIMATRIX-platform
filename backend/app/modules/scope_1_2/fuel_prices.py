...
FUEL_PRICES = {
    ...
    "district_heating": {
        "Global": {
            ...
            "currency": "USD",
            "unit": "kWh",
            "source": "IEA_District_Heat",
        },
        "EU": {
            "price": Decimal("0.07"),
            "currency": "EUR",
            "unit": "kWh",
            "source": "Euroheat_Power",
        },
        "UK": {
            "price": Decimal("0.06"),
            "currency": "GBP",
...
def convert_spend_to_quantity(
    fuel_type: str, spend_amount: Decimal, region: str = "Global"
) -> tuple:
    """
...
def get_all_prices_flat() -> list:
    ...
    for fuel_type, prices in FUEL_PRICES.items():
        ...
        if isinstance(prices, dict) and "price" in prices:
            ...
            result.append({"fuel_type": fuel_type, "region": "Global", **prices})
        else:
            # Multi-region prices
            for region, price_info in prices.items():
                result.append({"fuel_type": fuel_type, "region": region, **price_info})
    return result
