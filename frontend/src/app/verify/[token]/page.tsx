 ...
 *
 * One token unlocks exactly ONE scope, and the portal renders the matching
 * surface:
 * - a reporting period: the org's inventory with every line's source,
 *   factor, method and result, plus the audit trail;
 * - an EPD project: the declaration, its frozen EN 15804 results matrix
 *   and the per-line derivations behind it.
 * Read-only, one org, nothing else. This is the "audit-ready platform +
 * verifier read-only role" build item (docs/AUDIT-PARTNERS-RESEARCH.md):
 * the per-line derivation trail no competitor exposes, handed to the
 * verifier in one place — for corporate footprints and product
 * declarations alike.
 */
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#F4F7F5] p-6">
      <div className="max-w-md rounded-2xl border border-[#EDF2EE] bg-white p-8 text-center shadow-sm">
        ...
        <p className="mt-2 text-[13px] text-[#67716B]">{message}</p>
      </div>
    </div>
  );
}
...
function EpdPortal({ payload }: { payload: VerifierEpdPayload }) {
  const { epd, results, checklist } = payload;
  const lca = results?.lca ?? null;
  const lines = results?.line_items ?? [];
  ...
  const declRows: [string, string][] = [
    ...
    ['Status', epd.status.replace('_', ' ')],
    ['Registration number', epd.registration_number ?? '— pending'],
    ['Valid until', epd.valid_until ? epd.valid_until.slice(0, 10) : '— set on publication'],
  ];
  ...
  return (
    <PortalShell>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-[12px] font-semibold uppercase tracking-[0.06em] text-[#939C96]">
              {payload.organization_name} · Environmental Product Declaration
            </p>
            <h1 className="mt-0.5 text-[22px] font-bold tracking-[-0.01em]">{epd.name}</h1>
            <p className="mt-1 text-[12.5px] text-[#67716B]">
              {epd.product_name}
              {payload.verifier_name ? ` · prepared for ${payload.verifier_name}` : ''}
            </p>
          </div>
          <span
            className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[12px] font-semibold ${
              payload.results_are_frozen ? 'bg-[#E4F1EB] text-[#1F7A5C]' : 'bg-[#F6EEDC] text-[#A97614]'
            }`}
            title={
              payload.results_are_frozen
                ? 'Results froze when the declaration left draft — version-stable'
                : 'Results are a live preview — the declaration has not been frozen yet'
            }
          >
            <Snowflake className="h-3.5 w-3.5" />
            {payload.results_are_frozen ? 'Frozen declaration snapshot' : 'Live preview (not frozen)'}
          </span>
        </div>

        <div className="mt-5 grid gap-x-8 gap-y-2 border-t border-[#EDF2EE] pt-5 sm:grid-cols-2 lg:grid-cols-3">
          {declRows.map(([k, v]) => (
            <div key={k} className="text-[12.5px]">
              <span className="text-[#939C96]">{k}: </span>
              <span className="font-semibold capitalize">{v}</span>
            </div>
          ))}
          {results && (
            <div className="text-[12.5px]">
              <span className="text-[#939C96]">Climate (A1-A3): </span>
              <span className="font-semibold tabular-nums">
                {results.pcf.total_kgco2e_per_unit.toFixed(2)} kg CO2e
              </span>
              {results.pcf.primary_data_share != null && (
                <span className="text-[#67716B]"> · primary data {results.pcf.primary_data_share.toFixed(1)}%</span>
              )}
            </div>
          )}
        </div>
      ...
      {/* Readiness checklist */}
      <div className="mt-6 rounded-2xl border border-[#EDF2EE] bg-white p-6 shadow-sm">
        <h2 className="text-[13px] font-bold uppercase tracking-[0.06em] text-[#939C96]">Preparation status</h2>
        <ul className="mt-3 grid gap-2 sm:grid-cols-2">
          {checklist.map((c) => (
            <li key={c.key} className="flex items-start gap-2 text-[12.5px]">
              {c.ok ? (
                <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[#1F7A5C]" />
              ) : (
                <Circle className="mt-0.5 h-4 w-4 shrink-0 text-[#A97614]" />
              )}
              <div>
                <span className="font-semibold">{c.label}</span>
                <div className="text-[11.5px] text-[#67716B]">{c.detail}</div>
              </div>
            </li>
          ))}
        </ul>
      ...
      {lca && (
        <div className="mt-6 overflow-x-auto rounded-2xl border border-[#EDF2EE] bg-white p-6 shadow-sm">
          <h2 className="text-[13px] font-bold uppercase tracking-[0.06em] text-[#939C96]">
            EN 15804 results · {lca.method}
          </h2>
          <table className="mt-3 w-full min-w-[560px] border-collapse text-[12.5px]">
            <thead>
              <tr className="border-b border-[#EDF2EE]">
                <th className="py-2 pr-3 text-left text-[10.5px] font-bold uppercase tracking-[0.06em] text-[#939C96]">
                  Indicator
                </th>
                {lca.modules.map((m) => (
                  <th key={m} className="py-2 pr-3 text-right text-[10.5px] font-bold uppercase tracking-[0.06em] text-[#939C96]">
                    {m}
                  </th>
                ))}
                <th className="py-2 pr-3 text-right text-[10.5px] font-bold uppercase tracking-[0.06em] text-[#939C96]">
                  Total
                </th>
                <th className="py-2 text-left text-[10.5px] font-bold uppercase tracking-[0.06em] text-[#939C96]">
                  Unit
                </th>
              </tr>
            </thead>
            <tbody>
              {lca.rows.map((r) => (
                <tr key={r.code} className="border-b border-[#EDF2EE] last:border-0">
                  <td className="max-w-[15rem] py-1.5 pr-3 font-medium">{r.name}</td>
                  {lca.modules.map((m) => (
                    <td key={m} className="py-1.5 pr-3 text-right tabular-nums text-[#67716B]">
                      {fmtVal(r.by_module[m] ?? 0)}
                    </td>
                  ))}
                  <td className="py-1.5 pr-3 text-right font-semibold tabular-nums">{fmtVal(r.total)}</td>
                  <td className="py-1.5 text-[11px] text-[#939C96]">{r.unit}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <p className="mt-3 text-[11px] text-[#939C96]">{lca.note}</p>
        </div>
      )}

      {/* Per-line derivations */}
      {lines.length > 0 && (
        <div className="mt-6 overflow-x-auto rounded-2xl border border-[#EDF2EE] bg-white shadow-sm">
          <table className="w-full border-collapse text-[12.5px]">
            <thead>
              <tr className="border-b border-[#EDF2EE]">
                {['Input', 'Module', 'Quantity', 'Factor / source', 'Derivation', 'kg CO2e'].map((h) => (
                  <th key={h} className="px-4 py-2.5 text-left text-[10.5px] font-bold uppercase tracking-[0.06em] text-[#939C96]">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {lines.map((l) => (
                <tr key={l.input_id} className="border-b border-[#EDF2EE] align-top last:border-0">
                  ...
                  <td className="px-4 py-3 text-[#67716B]">
                    {l.factor ? (
                      <>
                        <div>{l.factor.display_name}</div>
                        <div className="text-[11px] text-[#939C96]">{l.factor.source}</div>
                      </>
                    ) : (
                      <span className="text-[#A97614]">Gap — no factor</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    {l.formula && <div className="font-mono text-[10.5px] text-[#939C96]">{l.formula}</div>}
                    {l.warnings.map((w, i) => (
                      <div key={i} className="text-[11px] text-[#A97614]">
                        {w}
                      </div>
                    ))}
                  </td>
    ...
    <PortalShell>
      ...
      <div className="rounded-2xl border border-[#EDF2EE] bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h1 className="mt-0.5 text-[22px] font-bold tracking-[-0.01em]">{period.period_name}</h1>
            <p className="mt-1 text-[12.5px] text-[#67716B]">
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            {period.assurance_level && (
              <span className="text-[11.5px] text-[#67716B]">Assurance: {period.assurance_level}</span>
            )}
            {period.verified_by && (
              <span className="text-[11.5px] text-[#67716B]">Verified by {period.verified_by}</span>
            )}
          </div>
        </div>
        ...
        {(['inventory', 'audit'] as Tab[]).map((t) => (
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-[12.5px] font-semibold transition-colors ${
              tab === t ? 'bg-[#1F7A5C] text-white' : 'bg-white text-[#67716B] hover:text-[#212724]'
            }`}
          >
              ...
              {lines.map((l) => (
                <tr key={l.id} className="border-b border-[#EDF2EE] align-top last:border-0">
                  ...
                  <td className="px-4 py-3">
                    ...
                    {l.formula && <div className="mt-0.5 font-mono text-[10.5px] text-[#939C96]">{l.formula}</div>}
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-[#67716B]" title={l.data_quality_justification || undefined}>
                      {l.data_quality_score != null ? `PCAF ${l.data_quality_score}/5` : '—'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right font-semibold tabular-nums">{tonnes(l.co2e_kg)}</td>
                </tr>
              ))}
              {lines.length === 0 && (
                <tr>
      ...
      {tab === 'audit' && (
        <div className="mt-4 overflow-hidden rounded-2xl border border-[#EDF2EE] bg-white shadow-sm">
          {auditLog.length === 0 ? (
            <p className="px-4 py-8 text-center text-[13px] text-[#939C96]">No audit entries recorded.</p>
          ) : (
...
export default function VerifierPortalPage() {
  ...
  const [epdPayload, setEpdPayload] = useState<VerifierEpdPayload | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const { kind } = await api.getVerifierContext(token);
      if (kind === 'epd') {
        setEpdPayload(await api.getVerifierEpd(token));
      } else {
  ...
  if (epdPayload) return <EpdPortal payload={epdPayload} />;
  if (error || !period) return <ErrorCard message={error ?? 'This verifier link is not valid.'} />;
  return <PeriodPortal period={period} lines={lines} auditLog={auditLog} />;
}
