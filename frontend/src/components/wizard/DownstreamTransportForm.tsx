...
/**
 * DownstreamTransportForm - Category 3.9 Downstream Transport & Distribution
 *
 * Supports 3 methods per GHG Protocol:
 * 1. Distance - Weight × distance based (tonne-km)
 * 2. Spend - Invoice/courier costs (USD)
 * 3. Supplier-Specific - User provides emission factor from carrier
 *
 * This covers outbound logistics - transportation of products TO customers AFTER sale.
 */
...
// Distance-based transport modes (outbound delivery focused)
const DISTANCE_MODES = [
  { key: 'van-delivery', label: 'Van (Last Mile Delivery)', activityKey: 'road_freight_van', efEstimate: 0.58946 },
  { key: 'road-hgv', label: 'HGV Truck', activityKey: 'road_freight_hgv', efEstimate: 0.10460 },
  { key: 'road-articulated', label: 'Articulated Truck', activityKey: 'road_freight_hgv', efEstimate: 0.09560 },
  { key: 'rail', label: 'Rail Freight', activityKey: 'rail_freight', efEstimate: 0.02780 },
  { key: 'sea-container', label: 'Sea Container', activityKey: 'sea_freight_container', efEstimate: 0.01612 },
  { key: 'sea-bulk', label: 'Sea Bulk', activityKey: 'sea_freight_bulk', efEstimate: 0.00354 },
  { key: 'air', label: 'Air Freight', activityKey: 'air_freight', efEstimate: 1.12820 },
  { key: 'courier', label: 'Courier/Parcel Service', activityKey: 'road_freight_van', efEstimate: 0.58946 },
  { key: 'multi-modal', label: 'Multi-modal', activityKey: 'road_freight_hgv', efEstimate: 0.10460 },
];

// Spend-based transport modes
const SPEND_MODES = [
  { key: 'road', label: 'Road/Truck Delivery', activityKey: 'freight_spend_road', efEstimate: 0.42 },
  { key: 'courier', label: 'Courier/Parcel Services', activityKey: 'freight_spend_road', efEstimate: 0.55 },
  { key: 'sea', label: 'Sea Freight', activityKey: 'freight_spend_sea', efEstimate: 0.18 },
  { key: 'air', label: 'Air Freight', activityKey: 'freight_spend_air', efEstimate: 1.85 },
  { key: 'rail', label: 'Rail Freight', activityKey: 'freight_spend_rail', efEstimate: 0.28 },
];
...
interface DownstreamTransportFormProps {
  periodId: string;
  onSuccess?: () => void;
}

export function DownstreamTransportForm({ periodId, onSuccess }: DownstreamTransportFormProps) {
  ...
  const preview = useMemo(() => {
    ...
    if (method === 'supplier-specific') {
      ...
      return {
        activityKey: 'supplier_specific_3_9',
        quantity: tonneKm,
        unit: 'tonne-km',
        co2e,
  ...
  const fullDescription = useMemo(() => {
    ...
    if (customerChannel) {
      parts.push(`[${customerChannel}]`);
    }
    if (destinationRegion) {
      const region = DESTINATION_REGIONS.find(r => r.key === destinationRegion);
      if (region) {
        parts.push(`(to ${region.label})`);
      }
    }
    return parts.filter(Boolean).join(' ');
  }, [description, customerChannel, destinationRegion]);
  ...
  const handleSave = async (addAnother: boolean = false) => {
    ...
    try {
      await createActivity.mutateAsync({
        ...
        description: fullDescription || `Downstream Transport: ${transportMode}`,
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-foreground-muted">
          Transportation of products TO customers AFTER sale (outbound logistics, deliveries)
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { value: 'distance' as TransportMethod, label: 'Distance', desc: 'Weight × Distance', Icon: Ruler, color: 'border-teal-500 bg-teal-50' },
            { value: 'spend' as TransportMethod, label: 'Spend', desc: 'Logistics Costs', Icon: DollarSign, color: 'border-green-500 bg-green-50' },
            { value: 'supplier-specific' as TransportMethod, label: 'Carrier', desc: 'Carrier-provided EF', Icon: ClipboardList, color: 'border-amber-500 bg-amber-50' },
          ].map((m) => (
            <button
              key={m.value}
              onClick={() => {
        ...
        <div className="space-y-4 p-4 bg-green-50 rounded-lg border border-green-200">
          <div>
            ...
            <select
              ...
              className={selectClass}
            >
              <option value="">Select logistics service...</option>
        ...
        <div className="space-y-4 p-4 bg-amber-50 rounded-lg border border-amber-200">
          <div className="flex items-start gap-2 p-3 bg-white rounded-lg border border-amber-300">
            ...
            <p className="text-sm text-amber-800">
              <strong>Carrier-Provided Method:</strong> Enter the emission factor provided by your
              delivery carrier (e.g., from their sustainability report, DHL GoGreen, UPS carbon calculator).
            </p>
          </div>
          ...
          <Input
            label="Carrier Emission Factor (kg CO2e per tonne-km)"
            type="number"
            step="0.00001"
            min="0"
            value={supplierEf}
            ...
            hint="Get this value from your carrier's carbon report or sustainability program"
          />
      ...
      {method && (
        <div className="space-y-4">
          <Input
            ...
            placeholder="e.g., Monthly customer deliveries"
          />

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-foreground mb-1.5">Destination Region</label>
              <select
            ...
            <Input
              label="Customer/Channel (optional)"
              value={customerChannel}
              onChange={(e) => setCustomerChannel(e.target.value)}
              placeholder="e.g., B2C E-commerce, Retail partners"
            />
          </div>
